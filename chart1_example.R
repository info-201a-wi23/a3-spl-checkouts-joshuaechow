...
x_and_y_values <- checkouts_df %>% group_by(MaterialType, CheckoutYear) %>% filter(MaterialType %in% c("BOOK", "EBOOK", "AUDIOBOOK", "VIDEODISC", "SOUNDDISC") & CheckoutYear < 2023) %>% summarize(Checkouts = sum(Checkouts))  
